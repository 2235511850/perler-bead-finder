// mard221.js —— MARD 标准 221 色色号 → HEX
// 数据来源：https://www.pindou.online/colors 与 https://www.pixel-beads.com/zh/mard-bead-color-chart 双向校对
// 注意：HEX 仅为屏幕参考值；实物颜色受光线、批次影响，但用作感知色差排序已足够。
// 全局暴露 window.MARD221 / window.MARD221_RGB
(function () {
  const HEX = {
    // A 系列（26 色）
  ...
  // 预解析成 RGB [r,g,b]，省去热路径转换
  const RGB = {};
  for (const code in HEX) {
    const n = parseInt(HEX[code].slice(1), 16);
    RGB[code] = [(n >> 16) & 255, (n >> 8) & 255, n & 255];
  }

  // 顺序数组（与 HE 颜色集合一致，便于按字母遍历）
  const CODES = Object.keys(HEX);

  window.MARD221 = HEX;
  window.MARD221_RGB = RGB;
  window.MARD221_CODES = CODES;
})();
